...
create_dscBLASTobject <- function(expression_matrix,query_species,ref_species='both',metadata,cell_type,batch='default',downsample=50000,mtx.type='raw') {
  if(ref_species!='both'){
    seob=Cell_qc(expression_matrix,query_species,metadata,cell_type,downsample)
    if(mtx.type=='raw'){
