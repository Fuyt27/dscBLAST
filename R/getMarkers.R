...
#' @export
#'
getMarkers <- function(species,reference,ref_celltype,ref_dataset,marker_top_n){
  if(species=='Hs'){
    if (reference=='ecto') {
  ...
  marker.final =list(marker.use,markerlist)
  names(marker.final)=c(paste0(ref_dataset,'_',ref_celltype,'_top',marker_top_n),'marker_total')
  return(marker.final)
}
